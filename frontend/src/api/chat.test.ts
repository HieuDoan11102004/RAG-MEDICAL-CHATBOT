...
describe("sendPrompt", () => {
  ...
  it("posts a prompt and returns a cited answer", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining("/api/messages"), expect.objectContaining({ method: "POST", body: JSON.stringify({ prompt: "How much water?", conversation_id: "conversation-1" }) }));
  });
